...
- `src/main.cpp` is the single entrypoint for both GUI and CLI flows. It installs file logging to `gif-editor.log`, parses CLI actions, always creates a `QApplication`, then drives the same `MainWindow` editing pipeline for interactive and headless usage.
